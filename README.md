...
**Language & currency:** Mongolian is the site's default language — a visitor starts in MN and English is a choice the switch remembers (`mms_locale`). Prices are *stored* in USD but *quoted* in tögrög in both languages, converted at the day's live rate and rounded down to the nearest thousand ₮. `GET /api/settings` serves that rate: `services/exchangeRate.js` reads USD→MNT from a key-less public feed (open.er-api.com, falling back to currency-api), caches it for `FX_REFRESH_MINUTES` (default 360) and writes the last good reading to `settings.mnt_rate_live` so a restart or a provider outage still has yesterday's number. A stale reading is served immediately and refreshed in the background, so a slow provider never delays a page. Rates outside 1,000–10,000 ₮/$ are rejected as provider bugs rather than repriced into the shop. If nothing is reachable the admin-entered `mnt_rate` (`/admin/settings`) is used, and with no rate at all prices fall back to USD rather than showing a wrong conversion. Order confirmation emails quote the same converted figures.
